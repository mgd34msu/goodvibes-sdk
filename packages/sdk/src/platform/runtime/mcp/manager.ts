...
export class McpLifecycleManager {
   ...
   * unsupported state). Emits `MCP_SCHEMA_QUARANTINED`.
   *
   * @param serverName - Server identifier
  ...
  private _scheduleReconnect(serverName: string): void {
    ...
    const timer = setTimeout(() => {
      ...
      void this._connect(serverName).catch((error: unknown) => {
        logger.warn('McpLifecycleManager: reconnect attempt failed', {
          server: serverName,

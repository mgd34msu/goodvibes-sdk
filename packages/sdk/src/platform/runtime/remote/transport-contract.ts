...
/**
 * Protocol support matrix for v1.x of the transport protocol.
 *
...
export const TRANSPORT_PROTOCOL_SUPPORT_MATRIX: ProtocolSupportMatrix = Object.freeze([
  Object.freeze<ProtocolSupportEntry>({
...
export function negotiateProtocolVersion(
  ...
  peerVersion: Readonly<ProtocolVersion>,
  matrix: ProtocolSupportMatrix = TRANSPORT_PROTOCOL_SUPPORT_MATRIX,
): VersionNegotiationResult {
  // Rule 1: Major mismatch → always unsupported
  if (localVersion.major !== peerVersion.major) {
    return {
      proceed: false,
      unsupportedCode: 'major_version_mismatch',
      unsupportedReason:
  ...
  // Find the protocol support entry for the local version
  ...
  if (peerVersion.minor < minMinor) {
    return {
      proceed: false,
      unsupportedCode: 'peer_version_too_old',
      unsupportedReason:
...
export interface ControlPayloads {
  HANDSHAKE_INIT: {
    ...
    /** Structured version object for server-side protocol support matrix lookup. */
  ...
  HANDSHAKE_REJECT: {
    ...
    /**
     * Structured unsupported code when the reject is due to version mismatch.

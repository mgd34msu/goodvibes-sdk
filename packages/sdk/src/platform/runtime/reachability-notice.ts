 ...
 * build that knows it is not the current one must say so before anything else,
 * so this module turns those two facts into lines and reachability-check.ts
 * next door hands them to whatever a product prints with.
 *
 * Pure by construction: it is handed a completed scan and two version strings
 * and decides only what to say.
 */

import { compareVersions, normalizeVersion } from './self-update.js';
import { describeShadowReport, type ShadowScanResult } from './path-shadow.js';

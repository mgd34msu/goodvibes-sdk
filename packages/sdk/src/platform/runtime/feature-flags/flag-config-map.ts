...
export const FEATURE_FLAG_CONFIG: Readonly<Record<string, FeatureFlagConfigAssociation>> = {
  ...
  // ── Voice ────────────────────────────────────────────────────────────────
  // Every voice.wake.* row except the enablement key itself, so a surface
  // renders the wake word's detection tuning, capture, feedback, retention,
  // supervision and per-surface delivery as ONE configurable unit.
  'wake-word-detection': {
    configCategories: ['voice'],
    configKeys: [
      'voice.wake.models',
  ...
  'paired-device-capabilities': {
    configCategories: ['device'],
    configKeys: [

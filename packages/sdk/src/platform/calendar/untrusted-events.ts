 ...
 * `SubscriptionStore.events()` / `allEvents()` record; `refresh()` and
 * `applyFetch()` — which are arrival — record nothing, and must not start.
 ...
 *
 * Ruled, not inferred: an .ics import, a CalDAV collection and a subscribed
 * feed are external unconditionally, and a provider event is external unless
 * the provider itself says the owner organized it. An event the owner created
 * himself is not external.
 */
...
function organizerLabel(organizer: string | undefined): string {

...
export function deriveRepairProfileFacts(input: {
  ...
  return PROFILE_RULES.flatMap((rule) => {
    const exactValues = rule.title === 'Audio capabilities'
      ? Array.from(text.matchAll(/\b\d+\s*x\s*\d+\s*w\b/gi)).map((match) => match[0])
      : [];
    ...
    const minimumMatches = broadProfileIntent || rule.intent.test(query) ? 1 : rule.minimumMatches;

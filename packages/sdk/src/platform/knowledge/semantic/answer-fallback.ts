...
function collectProfilePhrases(
  ...
  for (const item of evidence) {
    ...
    const title = readEvidenceTitle(item);
    if (title) phrases.push(title);
  }
  return uniqueCaseInsensitive(phrases);
}

function readEvidenceTitle(item: AnswerFallbackEvidence): string {
  const raw = normalizeWhitespace(item.title ?? '');
  if (!raw || !hasConcreteFeatureText(raw)) return '';
  const cleaned = normalizeWhitespace(clampText(stripSourceAddressFragments(raw), 220));
  if (isRawSourceFragment(cleaned)) return '';
...
function hasConcreteFeatureText(value: string): boolean {
  return /\b(hdmi|usb|hdr|hdr10|dolby|vision|earc|arc|bluetooth|wi-?fi|ethernet|voice|remote|game|filmmaker|nanocell|nano\s*cell|4k|uhd|resolution|refresh|ports?|speakers?|audio|display|screen|webos|atsc|ntsc|qam|tuner)\b/i.test(value);
}
...
function featureProfileTerms(value: string, category: FeatureProfileCategory): string[] {
  const termMap: Record<FeatureProfileCategory, readonly [string, RegExp][]> = {
    display: [
      ...
      ['NanoCell display', /\bnano\s*cell\b|\bnanocell\b/i],

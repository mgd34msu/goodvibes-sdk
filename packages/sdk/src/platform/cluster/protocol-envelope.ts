...
/** The group-scoped digest shape produced by digestSurfaceId. */
const GROUP_SURFACE_ID_PATTERN = /^s[0-9a-f]{32}$/;

/** Either digest shape that may legally appear as an envelope's `surfaceId`. */
function isWireSurfaceId(value: unknown): value is string {
  if (typeof value !== 'string') return false;
  return isSurfaceId(value) || GROUP_SURFACE_ID_PATTERN.test(value);
}
...
function readEnvelopeFields(candidate: Record<string, unknown>): ClusterEnvelope | null {
  ...
  if (surfaceId !== null && !isWireSurfaceId(surfaceId)) return null;

...
export class GroupWireRouter {
   ...
   *
   * `type`, `nodeId`, `version` and `seq` become envelope fields (the wire
   * format has one home for each of those). Everything else the election put on
   * the message — its uptime, its own optional signature, the surface it is
   * about — rides in `body` untouched, so the message that comes out the far
   * side is byte-for-byte the one that went in.
   */
      ...
      return encodeEnvelope(
        {
          type,
          nodeId,
          nodeVersion: typeof message['version'] === 'string' ? message['version'] : nodeVersion,

 ...
 * (name · created · last-seen · id), so no two of them drift. Pure over its
 * inputs and a `now` clock, so the formatting is testable without real time.
 */
import type { PublicPairingToken } from './pairing-token-store.js';

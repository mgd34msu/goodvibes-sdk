 ...
 */
import type { PaymentsConfig } from '../config/schema-types-payments.js';
 ...
 *
 * ── The owner settled this ────────────────────────────────────────────────
 *
 * On a marketplace the checkout domain is a household name while the actual
 * seller is anonymous. He ruled on exactly that case, and the recourse principle
 * is the reason:
 *
 *   "even etsy is fine, mainly because they have consumer protections"
 *
 * So `'major'` is the SETTLED default, not a placeholder: the card goes to the
 * marketplace, the marketplace holds the payment and the dispute, and that is
 * the accountability the rule is reaching for. Etsy and Amazon Marketplace
 * third-party sales qualify on this reasoning — the protection is the
 * marketplace's, so who the individual seller is does not change the remedy.
 *
 * eBay is the exception he carved out himself, and it is not expressed here: it
 * carries `'marketplace-per-seller'` because on eBay the recourse really does
 * depend on the seller, so the listing is graded per-listing in
 * `marketplace-listing.ts`.
 *
 * ── The other values, and the trap in one of them ─────────────────────────
 *
 ...
 * third-party ones. Both are owner policy edits rather than new code, and both
 * are STRICTER than the default — consistent with the rule that policy may
 * tighten this decision and never loosen it.
 *
 * **The trap in `'first-party-only'`:** `saleType` is read off the page, so a
 * hostile listing would simply claim `'first-party'`. That is why `'unknown'`
 * counts as third-party, and why turning on `'first-party-only'` would first
 * require the saleType to come from a trusted extractor rather than page text.
 * Until then the field is carried and not trusted.
 */
...
export interface MajorRetailerPolicy {
  ...
  /** Defaults to `'major'` — his settled ruling. The other values only tighten. */
...
/**
 * Build the policy from the owner's settings.
 *
 * This exists so there is exactly ONE place that knows which config keys carry
 * the merchant standard. Every consumer — the daemon's checkout path, the
 * control-plane handlers, a surface previewing a decision — calls this rather
 * than reading `payments.majorRetailersAdditional` for itself. A second copy of
 * this mapping is how a renamed key silently degrades a purchase gate to its
 * defaults while still looking configured.
 *
 * `PaymentsConfig` is IMPORTED from the config schema, never restated here: a
 * local mirror of those field names is the same drift by another route.
 *
 * Note what is deliberately absent: there is no config key that makes this
 * decision more permissive. The knobs add domains he vouches for, remove
 * domains he does not, and move the eBay seller bar. Nothing here can turn an
 * unrecognised storefront into one that proceeds on silence.
 */
export function merchantPolicyFromConfig(config: PaymentsConfig): MajorRetailerPolicy {
  return {

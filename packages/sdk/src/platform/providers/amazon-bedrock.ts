...
// generation. Bedrock model rollout typically trails the direct Anthropic API
// by weeks; these two entries are NOT live-verified against a Bedrock account
// (no AWS credentials were available in this environment) — only against the
// direct Anthropic API's /v1/models. Existing entries are kept unchanged so
// nothing regresses if the newest generation isn't live on Bedrock yet.

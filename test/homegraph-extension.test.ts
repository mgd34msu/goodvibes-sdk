...
describe('Home Graph knowledge extension', () => {
  test('registers concrete Home Assistant object profiles on the base knowledge seam', () => {
    ...
    expect(HOME_GRAPH_KNOWLEDGE_EXTENSION.objectProfiles).toBeDefined();
    const objectProfiles = HOME_GRAPH_KNOWLEDGE_EXTENSION.objectProfiles!;
    expect(objectProfiles.map((profile) => profile.id)).toEqual([
      'homegraph-device',
      'homegraph-entity',
      'homegraph-integration',
    ]);
    expect(objectProfiles[0]?.subjectKinds).toContain('ha_device');
    expect(objectProfiles[0]?.suppressedGapKinds).toContain('battery');
  });
});

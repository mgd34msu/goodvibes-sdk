...
describe('telegram adapter — contract surface', () => {
  test('telegram adapter module is importable without throwing', async () => {
    await expect(import('../packages/sdk/src/_internal/platform/adapters/telegram/index.js')).resolves.toBeDefined();
  });

  test('telegram adapter module exports at least one function', async () => {
    const mod = await import('../packages/sdk/src/_internal/platform/adapters/telegram/index.js');

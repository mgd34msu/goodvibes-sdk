...
describe('the two halves of "inert until the payments branch merges", now that it has', () => {
  test('daemon.timezone resolves from the profile, as it did before the key existed', async () => {
    ...
    // See owner-profile-composition-readiness.test.ts: `daemon.timezone` is
    // not in ConfigKey on this tree, and `as never` matched no `get` overload
    // rather than widening the key. Widened at the manager instead.
    const anyKeyConfig2 = config2 as unknown as { get(key: string): unknown };

...
import {
  ...
  trustGatedAsk,
} from '../packages/sdk/src/platform/runtime/workspace-trust.ts';
...
describe('detectPriorWorkspaceState', () => {
  it('is false for a workspace with no .goodvibes', () => {
    expect(detectPriorWorkspaceState(workspace, SURFACE_ROOT)).toBe(false);
  });
...
describe('WorkspaceTrustManager', () => {
  it('a new workspace is undecided and restricted: reads allowed, writes denied', async () => {
    const mgr = new WorkspaceTrustManager({ shellPaths: makePaths(workspace), surfaceRoot: SURFACE_ROOT });
    await mgr.load();
  ...
  it('setLevel(trusted) allows all categories and persists across reloads', async () => {
    ...
    const reloaded = new WorkspaceTrustManager({ shellPaths: makePaths(workspace), surfaceRoot: SURFACE_ROOT });
    await reloaded.load();
  ...
  it('an already-persisted grandfathered decision (from before this fix) still reads honestly', async () => {
    ...
    writeFileSync(
      join(workspace, '.goodvibes', SURFACE_ROOT, 'trust.json'),

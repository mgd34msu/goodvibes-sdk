...
function caldavService(http: CalDavHttpPort, record?: CalendarUntrustedIngestRecorder) {
...
describe('externally-sourced event content', () => {
  test('an .ics import, a CalDAV collection and a subscription are external unconditionally', () => {
...
describe('a subscription poll records nothing; reading it records', () => {
  ...
  test('events() — a read someone asked for — records, with the feed URL masked', async () => {
    const ledger = new UntrustedContentLedger();
    const subscriptions = store(recorderInto(ledger), feedBody('Standup'));
    await subscriptions.add({ url: FEED_URL });
    expect(ledger.all()).toEqual([]);

    subscriptions.events('Shared Team Calendar');
  ...
  test('allEvents() records every subscription it hands back', async () => {
    const ledger = new UntrustedContentLedger();
    const subscriptions = store(recorderInto(ledger), feedBody('Standup'));
...
describe('no calendar module can initiate work', () => {
  ...
  test('the arrival path of the subscription store records nothing', () => {
    ...
    // from events()/allEvents(). If a future change calls the recorder from
    // applyFetch/refresh, this count goes up and this test fails.
    const recorderCalls = source.match(/recordCalendarEventIngest\s*\(/g) ?? [];
    expect(recorderCalls).toHaveLength(1);

    const applyFetch = source.slice(source.indexOf('private applyFetch'), source.indexOf('async refreshDue'));

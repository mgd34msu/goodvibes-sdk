    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (seed, goneCount) => {
          ...
          // Read out and narrowed rather than indexed inline: under
          // noUncheckedIndexedAccess the index yields `number | undefined`,
          // which `toBe` will not take against a `number`.
          const lastGone = gone.at(-1);
          if (lastGone === undefined) throw new Error('the fixture generated no vanished UIDs');
          expect(report.cursor.lastSeenUid).toBe(lastGone);

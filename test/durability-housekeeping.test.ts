...
import { logger } from '../packages/sdk/src/platform/utils/index.ts';
...
import { QUARANTINE_RETENTION_MS, UNRECOGNIZED_SUFFIX } from '../packages/sdk/src/platform/config/read-versioned.ts';
import { ANCHOR_SIDECAR_SETTLE_MS } from '../packages/sdk/src/platform/rewind/turn-anchors.ts';
import { JOURNAL_ORPHAN_MAX_AGE_MS, journalPathFor } from '../packages/sdk/src/platform/runtime/transcript-journal.ts';
import { LIVENESS_STALE_AFTER_MS, livenessMarkerDirFor, livenessMarkerPathFor } from '../packages/sdk/src/platform/runtime/session-liveness-marker.ts';
import {
  runDurabilityHousekeeping,
  startDurabilityHousekeeping,
} from '../packages/sdk/src/platform/runtime/durability-housekeeping.ts';
...
beforeEach(() => {
  ...
  mkdirSync(join(tmpDir, '.goodvibes', TEST_SURFACE_ROOT), { recursive: true });
...
function putOldQuarantine(name: string): string {
  const path = join(tmpDir, '.goodvibes', TEST_SURFACE_ROOT, `${name}${UNRECOGNIZED_SUFFIX}`);

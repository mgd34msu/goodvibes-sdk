...
/**
 * OBS-03: instrumentedFetch migration — verifies instrumentedFetch has the same
...
describe('obs-03 instrumentedFetch migration', () => {
  ...
  test('sanitizeUrlForLog redacts sensitive query params', async () => {
    const { sanitizeUrlForLog } = await import('../packages/sdk/src/_internal/platform/utils/fetch-with-timeout.js');

...
describe('the day boundary behaves as ruled', () => {
  test('the midnight split is accepted: 23:59 and 00:00 are different days', () => {
    ...
    expect(String(dayKey(before, tz))).toBe('2026-07-27');
    expect(String(dayKey(after, tz))).toBe('2026-07-28');
  });
  ...
  test('unset timezone means UTC', () => {
    ...
    expect(String(dayKey(atMs, ''))).toBe('2026-07-27');
    expect(String(dayKey(atMs, 'not/a-real-zone'))).toBe('2026-07-27');
  });

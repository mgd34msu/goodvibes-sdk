 ...
 * `parseCurrencyCode` is the real parser, which returns null on anything it
 * does not recognise.
 */
import {
  parseCurrencyCode,
...
/** The stored shipping address every fixture purchase ships to. */
const SHIPPING_ADDRESS: PostalAddress = {
...
function buildHarness(options: HarnessOptions): Harness {
  ...
  const deps: CheckoutFlowDeps = {
    ...
    merchantJudge,
    limits,
    budgetCurrency: currency(merchant.currency === 'EUR' ? 'EUR' : 'USD'),
    timezone: 'UTC',
    gates: {
      enabled: true,
      hasUsableCard: true,
...
describe('the card never comes back', () => {
  test('the control-plane response carries field NAMES and a boolean, nothing else', async () => {
    ...
    await registry.open({
      ...
      merchantDomain: 'bestbuy.com', cardId: 'card-1', item: unsafeOwnerSuppliedTextForTests('thing'), currency: currency('USD'),
    ...
    const service = {
      async fillCardIntoCheckout(input: Parameters<PaymentsGatewayService['fillCardIntoCheckout']>[0]): Promise<PaymentFillCardResult> {
        return fillCard(
          {
    ...
    } as unknown as Parameters<typeof handler>[0]) as Record<string, unknown>;
  ...
  test('the route rebuilds the response from an allowlist, so a leaky service loses the leak', async () => {
    ...
    } as unknown as Parameters<ReturnType<typeof createPaymentsCheckoutFillCardHandler>>[0]) as Record<string, unknown>;
...
describe('the daemon computes every number the owner acts on', () => {
  for (const shape of shapes()) {
    test(`parses ${shape.name} into integer minor units`, async () => {
      const merchant = shape.get();
      const reading = await readFixtureCheckout(merchant);
      const extraction = extractCheckout(reading, currency(merchant.currency === 'EUR' ? 'EUR' : 'USD'));
  ...
  test('a merchant total that disagrees with its own line items stops the purchase', async () => {
    ...
    const extraction = extractCheckout(lied, currency('USD'));
...
describe('when the merchant interrupts or the process dies', () => {
  ...
  test('what a restart tells him distinguishes the three cases in plain words', () => {
    const record = {
      ...
      cardId: 'card-1', item: unsafeOwnerSuppliedTextForTests('thing'), currency: currency('USD'), phase: 'submit-pending' as const,

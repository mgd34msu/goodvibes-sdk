...
import type { ImapEnvelope } from '../packages/sdk/src/platform/email/imap-types.ts';
...
function envelopeFor(uid: number): ImapEnvelope {
  return {
    ...
    deliveryEvidence: [{ address: 'watched@example.test', rawValue: 'watched@example.test', source: 'delivered-to' as const }],
...
async function build(): Promise<Harness> {
  ...
  const sink = new DedupingInboundMailSink({
    ...
    handle: async (raw: InboundMailboxMessage) => {
      const msg = requireImapMessage(raw, 'dedup handle');
...
describe('a redelivered message produces exactly one notice', () => {
  ...
  test('a suppressed duplicate resolves rather than throwing', async () => {
    ...
    const sink = new DedupingInboundMailSink({
      dedup,
      handle: async (raw) => { handled.push(requireImapMessage(raw, 'dedup handle').uid); },
    });
    const msg = imapMessageFixture({
      account: ACCOUNT, mailbox: MAILBOX, uidValidity: 42, uid: 7, via: 'idle',
    });
  ...
  test('a suppressed duplicate still ADVANCES THE CURSOR, so the mailbox drains', async () => {
    ...
    await sink.deliver(imapMessageFixture({
      account: ACCOUNT, mailbox: MAILBOX, uidValidity: UID_VALIDITY, uid: 102, via: 'idle',
    }));
  ...
  test('two concurrent deliveries of one message run the pipeline once', async () => {
    ...
    const dedup = createInboundMailDedup();
    const handled: number[] = [];
    // An array rather than a nullable let: the assignment happens inside the
    // handle closure, which TypeScript's flow analysis cannot see, so a
    // `let release: (() => void) | null` stays narrowed to `null` at the call
    // below and is not callable. Pushing into a list sidesteps the narrowing
    ...
    const msg = imapMessageFixture({
      account: ACCOUNT, mailbox: MAILBOX, uidValidity: 42, uid: 9, via: 'idle',
    });

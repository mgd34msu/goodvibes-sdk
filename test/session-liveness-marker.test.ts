...
import {
  ...
  writeLivenessMarker,
} from '../packages/sdk/src/platform/runtime/session-liveness-marker.ts';
...
describe('livenessMarkerPathFor', () => {
  test('builds the canonical path under the surface root\'s liveness directory', () => {
    const path = livenessMarkerPathFor(makeTestSurface('/home/user'), 'ses-abc');
    expect(path).toBe(`/home/user/.goodvibes/${TEST_SURFACE_ROOT}/liveness/ses-abc.json`);
  });
});

 ...
 *
 * Typed `string[]` and not `readonly string[]`: bun's scalar `test.each`
 * overload is `each<const T>(table: T[])`, so a readonly array matches no
 * overload and every row parameter silently degrades to `unknown`.
 */
const DAEMON_NEEDED_SAMPLE: string[] = [
  'SLACK_BOT_TOKEN',
  'DISCORD_BOT_TOKEN',
  'NTFY_ACCESS_TOKEN',
...
// `rmSync` is already imported at the top of the file; re-importing it here
// is a duplicate identifier in the same module scope.

...
describe('historyListDelta: scope gate', () => {
  ...
  test('a token with only gmail.metadata scope (the narrowest of the four) still passes the gate', async () => {
    const page = historyPage({ historyId: '101', addedMessageIds: [] });
    const { port } = fakeFetchPort({ history: [jsonResponse(200, page)] });

 ...
 *
 * Ported from the terminal app's suite when this module was hoisted.
 */
import { describe, expect, test } from 'bun:test';
import { createTasksClient } from '../packages/sdk/src/platform/runtime/client/tasks-client.ts';
import type { DaemonVerbCaller } from '../packages/sdk/src/platform/runtime/client/daemon-verbs.ts';
import type { RuntimeTask } from '../packages/sdk/src/platform/runtime/store/index.ts';
...
function localSource(tasks: readonly RuntimeTask[]): { list: (limit: number) => readonly RuntimeTask[]; get: (id: string) => RuntimeTask | null } {
  return {
    list: () => tasks,
...
function verbs(
  ...
  options: { unavailable?: string; throws?: boolean } = {},
): DaemonVerbCaller & { calls: [string, unknown][] } {
...
describe('the task list is both registries', () => {
  test('local and daemon rows are unioned', async () => {
    const client = createTasksClient({
      ...
      verbs: verbs({ 'tasks.list': { tasks: [task('d1', 'scheduled work')] } }),
    });
  ...
  test('an id both registries carry appears once, from the local half', async () => {
    const client = createTasksClient({
      ...
      verbs: verbs({ 'tasks.list': { tasks: [task('shared', 'stale daemon title')] } }),
    });
    const result = await client.list();
  ...
  test('no daemon configured is reported as such, not as a failure', async () => {
    const client = createTasksClient({
      local: localSource([task('l1', 'local work')]),
      verbs: verbs({}, { unavailable: 'the daemon is disabled (daemon.enabled=false)' }),
    });
    const result = await client.list();
    expect(result.tasks).toHaveLength(1);
...
describe('one task, and which registry owns it', () => {
  test('a local id resolves locally and is never asked of the daemon', async () => {
    ...
    const client = createTasksClient({ local: localSource([task('l1', 'local work')]), verbs: caller });
  ...
  test('an unknown id falls through to the daemon', async () => {
    ...
    const client = createTasksClient({ local: localSource([]), verbs: caller });
  ...
  test('an id neither registry has is null, not a fabricated record', async () => {
    const client = createTasksClient({ local: localSource([]), verbs: verbs({ 'tasks.get': {} }) });
...
describe('acting on a daemon-owned task', () => {
  ...
  test('with no daemon reachable the act REJECTS rather than reporting success', async () => {
    const client = createTasksClient({
      local: localSource([]),
      verbs: verbs({}, { unavailable: 'no control-plane base URL is configured' }),
    });

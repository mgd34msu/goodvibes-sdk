...
} from '../packages/sdk/src/platform/daemon/safe-serve.ts';
...
describe('safe host serve factory', () => {
  ...
  test('propagates a bind-time failure (e.g. a real port conflict) unchanged, never swallowed as a request failure', () => {
    const bindError = Object.assign(new Error('address already in use'), { code: 'EADDRINUSE' });
    const baseServeFactory = mock(() => {
      throw bindError;
    });
    const serveFactory = createSafeHostServeFactory('test daemon', baseServeFactory as unknown as typeof Bun.serve);
    ...
    expect(() => serveFactory({
      port: 3421,
      fetch: () => Response.json({ ok: true }),
    } as Parameters<typeof Bun.serve>[0])).toThrow(bindError);
  });

  test('passes options through untouched when no fetch handler is supplied', () => {
    const baseServeFactory = mock((options: unknown) => options);
    const serveFactory = createSafeHostServeFactory('test daemon', baseServeFactory as unknown as typeof Bun.serve);

    const options = { port: 0 } as Parameters<typeof Bun.serve>[0];
    serveFactory(options);

    expect(baseServeFactory).toHaveBeenCalledWith(options);
  });
});

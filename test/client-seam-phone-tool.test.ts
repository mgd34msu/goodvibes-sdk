...
import { GatewayMethodCatalog, registerDevicesGatewayMethods } from '../packages/sdk/src/platform/control-plane/index.ts';
import {
  DEVICE_CAPABILITY_CONTRACT_VERSION,
  DEVICE_CAPABILITY_IDS,
  DEVICE_NODE_ANNOUNCEMENT_KEY,
  createDevicePostureRuntime,
} from '../packages/sdk/src/platform/devices/index.ts';
import type { DeviceApprovalBridge, DevicePeerTransport, DevicePeerView } from '../packages/sdk/src/platform/devices/index.ts';
import { createDevicesClient } from '../packages/sdk/src/platform/runtime/client/devices-client.ts';
import { createClientPhoneTool } from '../packages/sdk/src/platform/runtime/client/phone-tool.ts';
...
function harness(): Harness {
  const root = mkdtempSync(join(tmpdir(), 'gv-phone-verbs-'));
  roots.push(root);
  const asks: string[] = [];
  ...
  // The hoisted client shape, over the real handlers. `probe` reports a

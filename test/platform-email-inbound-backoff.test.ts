...
describe('capability verdicts', () => {
  ...
  test('a refused credential is insufficient and terminal, with an owner message', () => {
    ...
    // Narrowed rather than compared through an optional chain: the two texts
    // must be the SAME string, and `toBe` against `string | undefined` would
    // also be satisfied by there being no notice at all.
    expect(result.notice, 'a notice is what carries the owner-facing text').not.toBeNull();
    if (result.notice == null) throw new Error('unreachable: asserted defined above');

...
describe('Home Graph daemon routes', () => {
  ...
  test('exposes daemon routes for Home Graph clients', async () => {
    ...
    const exportedSourceCount = exported.sources?.length ?? 0;
    expect(exportedSourceCount).toBeGreaterThanOrEqual(1);
    expect(resetDryRun.dryRun).toBe(true);
    expect(resetDryRun.deleted?.sources).toBe(exportedSourceCount);
    expect(resetDryRun.deleted?.nodes).toBe(status.nodeCount);
    expect(resetDryRun.artifactDeleteCandidates).toBeGreaterThanOrEqual(1);
    ...
    expect(reset.deleted?.sources).toBe(exportedSourceCount);
    expect(reset.deleted?.nodes).toBe(status.nodeCount);
    expect(reset.artifactsDeleted).toBe(true);
    expect(reset.deletedArtifacts).toBeGreaterThanOrEqual(1);

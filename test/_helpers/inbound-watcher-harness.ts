 ...
 *
 * Checked on the discriminant rather than cast: wiring a Gmail source into
 * this harness is a mistake worth hearing about at the point it happens,
 * rather than one that surfaces later as `undefined` read out of a field that
 * variant never had.
 */
export function requireImapMessage(message: InboundMailboxMessage, where: string): ImapInboundMessage {
  if (message.source !== 'imap') {
    throw new Error(`${where}: this harness records IMAP messages, but was handed source="${message.source}"`);
  }
  return message;
}

/**
 * A complete, valid `ImapInboundMessage` for tests that need to hand one to a
 * sink or a dedup directly.
 *
 ...
 * Everything here is a real value of the real type; `overrides` covers the one
 * or two fields a given test actually cares about.
 */
export function imapMessageFixture(
  overrides: Partial<ImapInboundMessage> & { readonly uid: number },
): ImapInboundMessage {
  const uid = overrides.uid;
  const envelope: ImapInboundMessage['envelope'] = {
    uid,
    from: 'sender@example.test',
    subject: `fixture message ${String(uid)}`,
    date: 'Mon, 27 Jul 2026 12:00:00 +0000',
    messageId: `<fixture-${String(uid)}@example.test>`,
    mailbox: 'INBOX',
...
export class RecordingSink implements InboundMailSink {
  ...
  async deliver(message: InboundMailboxMessage): Promise<void> {
    const imap = requireImapMessage(message, 'RecordingSink.deliver');
    this.attempts.push(imap.uid);
    if (this.refuseOnce.has(imap.uid)) {
      this.refuseOnce.delete(imap.uid);
      throw new Error(`the sink refused UID ${imap.uid}`);
    }
    this.delivered.push(imap);
  }

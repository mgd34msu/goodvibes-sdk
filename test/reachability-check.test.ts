...
} from '../packages/sdk/src/platform/runtime/reachability-check.ts';
import { buildReachabilityNotices, reachabilityNoticeLines } from '../packages/sdk/src/platform/runtime/reachability-notice.ts';
import { scanCommandShadows } from '../packages/sdk/src/platform/runtime/path-shadow.ts';
...
describe('the startup check fires on an earlier-PATH copy', () => {
  test('it names both copies, both versions, and the exact fix', async () => {
    ...
    const result = await runReachabilityCheck({
      ...
      packageName: '@pellux/goodvibes-terminal',
...
describe('being behind is stated plainly, and only when it is known', () => {
  test('a package-managed install that cannot swap itself is told it is behind, with its own command', async () => {
    ...
    expect(text).toContain('bun add -g @pellux/goodvibes-terminal');
  });

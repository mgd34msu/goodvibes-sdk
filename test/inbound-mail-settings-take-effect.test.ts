...
import type { GmailSourceBuilder } from '../packages/sdk/src/platform/email/inbound/source-factory.ts';

/** Exactly what the factory hands the Gmail builder. */
type CapturedGmailInput = Parameters<GmailSourceBuilder>[0];
...
describe('the Gmail poll intervals come from the owner settings', () => {
  ...
  async function buildGmailWith(overrides: Record<string, unknown> = {}) {
    ...
    const captures: CapturedGmailInput[] = [];

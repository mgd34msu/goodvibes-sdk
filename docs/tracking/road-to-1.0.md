...
- **Current version target**: `0.25.13` (current pre-1.0 hardening line; 0.25.x carries dependency-audit remediation, ntfy route hardening, and ntfy chat reply-correlation fixes)
...
0.19.9          previous (Waves 1–9 consolidated + pipeline hardening + zero-any gate)
  ...
  → 0.25.13           current hardening line [CURRENT]
...
- [x] Decision: `./web` is sufficient for normal Worker-hosted operator HTTP clients. `./workers` was added later only for the optional GoodVibes Worker bridge around daemon batch proxying, queue consumers, and scheduled ticks.

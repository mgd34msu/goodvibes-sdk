...
flat list of ~307 dotted ids. Nothing enumerated or constrained the verb vocabulary, the final dotted segment of each id was a free-form word chosen independently by
...
the catalog (there is exactly one further outlier, `mcp.servers.remove`, see above), that is explicitly out of scope for a worst-class-only pass (risk #6, below).
...
**REJECTED** merging `automation.jobs.*` and `automation.schedules.*` into one family, that is a genuine product-capability question (does the agent's lighter
...
`opsApi.tasks` (the SDK's local, in-process runtime-task control plane, `platform/runtime/runtime-ops-api.ts`) and the wire `OPERATOR_METHOD_IDS` `tasks.*`

...
`/api/memory/doctor`) and `reviewQueue` already had `GET /api/memory/review-queue`, those are now surfaced through the client rather than re-routed.
...
wire transport type `MemoryTransport = MemoryCoreAccess & Partial<MemoryExtendedAccess>`, core required, extended OPTIONAL, so a surface pinned to an older SDK/daemon
...
break the single-writer invariant AND report a divergent local copy as canonical, the exact dishonest-recall failure the whole design prevents. This preserves the

...
not an owner ruling.** The owner ruling it enforces is this: card details are
entered only at a local terminal or the webui, never over a remote messaging
channel. The refusal below is the daemon-side enforcement of that ruling, and it
is recorded as a coordinator decision so nobody later cites it back as an owner
ruling in its own right.

The other half, that approvals and vetoes for purchases work over remote
channels, **is** an explicit owner ruling, and it stays.
...
| What an interceptor gains | knowledge of one decision, inferable from the outcome anyway | the ability to spend, anywhere, until the card is cancelled |
...
5. **The refusal reply is always delivered**, even though the content is
   ...
   objection inside a veto window elapses into a completed purchase. The owner is
   told immediately, on the same channel, and can resend without the digits.
...
it carries card shapes**. A veto is not counted just because a card number was
pasted alongside it. The refusal comes back immediately, naming the shapes, and
the veto can be resent on its own, which is why rule 5 exists and why the refusal
is never silent.
...
- A remote channel message is something the owner **typed**. Refusing it teaches
  the owner not to, and it can be resent. There is a person on the other end to
  tell.
- Mail is something a **merchant sent**, an order confirmation with the last
  four, or worse. Refusing it would discard evidence of a real purchase, from a
  sender who cannot be asked to resend. Redaction keeps the record and drops the
  digits.
...
Rejected, and it is not the coordinator's to rule: approvals and vetoes work over
remote channels by explicit owner ruling. Symmetry is not a reason to remove a
capability the owner asked for.

**Detect and redact card shapes on remote channels instead of refusing, as mail
does.** Rejected: redaction would silently accept the message, so the owner would
never learn that typing a card into Telegram is not a thing that works, and would
keep doing it.

Each attempt puts the digits into the channel's history before our redaction ever
sees them. The harm has already happened by the time the daemon reads the
message, so the response has to be one that changes the behavior rather than one

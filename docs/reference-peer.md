...
Generated from the synced GoodVibes peer contract for product version `0.25.10`.
...
Inspect distributed runtime pair requests, peers, work, and audit state.
...
Create a pending pair request and receive a challenge for operator approval.

- HTTP: `POST /api/remote/pair/request`
- Auth: `none`
- Required scope: none
...
Exchange an approved pair request and challenge for a scoped peer token.

- HTTP: `POST /api/remote/pair/verify`
- Auth: `none`
- Required scope: none
...
Report peer liveness, capability, command, version, and client-mode metadata.

- HTTP: `POST /api/remote/heartbeat`
- Auth: `bearer-peer-token`
- Required scope: `remote:heartbeat`
...
Complete, fail, or cancel a claimed work item.

- HTTP: `POST /api/remote/work/{workId}/complete`
- Auth: `bearer-peer-token`
- Required scope: `remote:complete`
...
Claim queued work for the authenticated peer.

- HTTP: `POST /api/remote/work/pull`
- Auth: `bearer-peer-token`
- Required scope: `remote:pull`
...
Node/device hosts are external processes. GoodVibes owns the pair/token/work protocol and can be controlled from web, channel, or daemon clients.

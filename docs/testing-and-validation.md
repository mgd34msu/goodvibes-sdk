...
because it is release/build output. There is not a separate license gate inside
`bun run validate` today. Run SBOM generation during release review or add a
dedicated license-policy step before treating license compliance as an automated
CI blocker.

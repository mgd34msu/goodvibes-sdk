...
`uuid.v4`, so this checked-in package provides that CommonJS `v4` surface using
...
when Verdaccio no longer pulls the current `@cypress/request` dependency path.

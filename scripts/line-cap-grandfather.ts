...
export const LINE_CAP_GRANDFATHER: Readonly<Record<string, GrandfatherEntry>> = {
  ...
  'packages/sdk/src/platform/config/schema-domain-runtime.ts': {
    ceiling: 837,
    justification: 'runtime config defaults + setting definitions; +30 for the relay.* reachability config (the relay defaults object plus the four relay.enabled/url/rendezvousId/label ConfigSettingDefinition entries); +7 for relay.requireStepUpForMutations (default + ConfigSettingDefinition entry) backing WebAuthn step-up on mutating relay calls',
  },

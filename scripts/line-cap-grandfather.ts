...
export const LINE_CAP_GRANDFATHER: Readonly<Record<string, GrandfatherEntry>> = {
  ...
  // checkout-flow.ts no longer has an entry: its PaymentNotifier and
  // PurchaseLedger ports split to payment-ports.ts (re-exported for
  // compatibility), taking the file back under the cap.
  ...
  // The file is ~759 lines, under the 800-line cap on its own merits.
  // wrfc-controller.ts ~2.97k, pre-split monolith, shrink-only
  'packages/sdk/src/platform/agents/wrfc-controller.ts': {
    ceiling: 2992,
    justification: 'pre-split monolith, shrink-only; +13 for the acceptance-checklist gate applied to BOTH review paths (the shared evaluateAcceptanceChecklistGate call, the unmet-items and missing-checklist critical-issue appends, and the pass-term additions in the main chain review and the compound-subtask review, gate mechanics live in completion-report.ts); +4 for stamping the gate-inclusive lastReviewVerdict onto the chain and subtask at their review gates, so wire consumers render the TRUE verdict rather than the reviewer claim; +9 for holding the chain-cleanup timers in an instance Set so dispose() can actually cancel them, the one-shot lived only in a local, leaving 38 uncancellable handles across a suite run',
  },
  ...
  'packages/daemon-sdk/src/knowledge-routes.ts': {
    ceiling: 1007,
    justification: 'daemon knowledge route surface, pre-split, shrink-only; +5 for readKnowledgeSubjectKind, which validates the usage/candidate targetKind and subjectKind query values instead of casting a raw string to the \'source\'|\'node\'|\'issue\' union',
  },
  ...
  'packages/sdk/src/platform/knowledge/service.ts': {
    ceiling: 929,
    justification: 'knowledge service facade, pre-split, shrink-only; +5 for the MemoryGovernor admission gate: the KnowledgeServiceConfig.admitExpensiveWork hook, the requireAdmission helper, and its terms on runJob/ingestUrl/ingestArtifact (honest structured refusal at the critical memory tier); +5 for making dispose() callable: it was private with no caller anywhere, so every composed graph leaked the schedule service\'s six reconcileSchedules timers per boot, the composition root now registers it for teardown',
  },
  ...
  'packages/sdk/src/platform/daemon/facade.ts': {
    ceiling: 955,
    justification: 'daemon facade, pre-split, shrink-only; +7 for owning the channel-health watcher\'s daemon lifecycle (import, field, construction, start after the cluster election, and stop on both the failed-start and normal-stop paths; the watcher and its announcer live in ./facade-channel-health.ts). Same argument as the trigger family above: a channel whose reported state is honest but which nothing sweeps is indistinguishable from one nobody checks. The defect this closes shipped, a Telegram bot whose ingress had died reported healthy because the state was computed from its token being present, so the owner sent a message, got no reply, and every surface agreed everything was fine; +9 for the approvals broker accessor (embedder/test seam) + doc; +18 for the outbound relay reachability boot seam (start/stop lifecycle wiring + surface accessor; the heavy composition lives in ../relay/daemon-wiring.ts); +24 for two in-process embedder seams the ACP adapter needs: cancelAgent (a real cancellation of a running agent) + registerMcpServer (connect a client-declared MCP server into the live registry) with docs; +6 for releasing sleep-inhibitor holds on a real (non-restart) stop, an exited daemon must never leave systemd-inhibit children blocking host sleep; +3 for surface ingress start/stop wiring, Telegram delivers nothing until the daemon calls setWebhook or getUpdates, so the route alone received no messages at all; the supervisor itself lives in ../channels/telegram/ingress.ts; +15 for owning the trigger family\'s daemon lifecycle: the triggerManager field + assignment, the start() call that recovers persisted triggers and arms the supervision tick, the shutdown() call on stop, and the `triggers` accessor (mirroring the `memory` accessor) an embedder or proof run needs. This wiring is the point of the subsystem: a trigger nothing starts is indistinguishable from a trigger that does not exist, and on-exit triggers are daemon-owned precisely so a six-hour build does not hold an agent turn open; +3 for making that dependency OPTIONAL after it broke every consumer that hand-composes RuntimeServices (goodvibes-agent builds its own object literal, so a required field turned \'this host has no trigger family\' into a TypeError on every daemon shutdown): the optional-chained start()/shutdown() calls and the absent-manager branch. Absence must mean no triggers, never a crash; +5 for the update-restart lifecycle: the crash-loop guard call at the top of start() (repeated boots that never reached a fully-started daemon restore the kept previous binary and hand over to it) plus the stopGracefully wiring that routes update and rollback restarts through the daemon\'s normal stop path instead of a bare exit, so shutdown hooks actually fire on a self-update; +6 for disposing the runtime graph on a real (non-restart) stop, gated on ownsRuntimeServices so a caller-supplied graph, which may outlive this daemon, is left alone: without it stop() returned with the config watch, fleet tick, memory governor, watcher registry and five more pollers still running; +4 for forwarding hasOverriddenHome to the lifecycle, so a daemon told to run out of a non-default home (--daemon-home, GOODVIBES_DAEMON_HOME, a test temp tree) cannot adopt the machine\'s service unit. One did: started from a scratchpad, it found the unit not running, wrote its own scratchpad ExecStart into the systemd unit and exited, and systemd supervised the throwaway as the machine\'s daemon for five hours while it read the real home\'s credentials. The guard itself is in facade-lifecycle.ts; this is the four-line pass-through; +9 for the hosted-session engine\'s daemon lifecycle: the manager field and its assignment, the restore-and-reconcile call at start (the report is stated by reportHostedSessionRestore in ./hosted-sessions-composition.ts, not spelled out here), and the stop call that ends kill-policy sessions and parks survive-policy ones with their transcripts. The composition itself is one line in facade-composition.ts and the engine is platform/hosted-sessions; what is here is only what a daemon must do at start and stop, which is exactly the wiring that goes missing silently; +3 for handing the hosted-session engine the SAME owner alerter the lifecycle already gets. A hosted session can be running with nobody attached, that is the whole point of the survive detach policy, so a message the daemon collected and then could not deliver into one has no screen anywhere to appear on. The alert path, its fallback ordering and its refusal logging are all in ./owner-alert.ts; this is the one call that connects them; +3 for reporting the running artifact\'s own release version on /status. The daemon ships from its own repository on its own release line, and /status answered every version read with the SDK build it happened to be composed against, so an updater comparing release tags and a client checking whether the daemon was new enough were reading a number about a different artifact. The host already states its artifact identity for the update loop; this hands the same value to the status surface; +4 for attaching update.status and update.check to the lifecycle that owns the self-update loop (one import, one call, two lines of doc). A daemon whose update checks had been failing for days stayed on an old build while three releases shipped, and every surface reported it healthy, because \'has not updated\' and \'cannot update\' produce identical silence. The verbs and their handlers live in ../control-plane/routes/update.ts and the state they read is the loop\'s own; what is here is the one call that can see both; +5 for exposing relay reachability over the wire. The controller was composed on every daemon that turns the relay on and reachable through exactly one door, getRelayReachability(), an in-process method on this facade, so a pure client had no verb to ask and could only report the relay unavailable. The accessor is passed as a FUNCTION because the controller is built at start and dropped at stop; a handler holding the instance would answer for a relay that is no longer running. The verbs and handlers live in ../control-plane/routes/relay.ts',
  },
  // facade-composition.ts (daemon) ~0.80k, the daemon's composition root; sat at
  // 797 and crossed the cap when hosted sessions were composed here.
  'packages/sdk/src/platform/daemon/facade-composition.ts': {
    ceiling: 802,
    justification: 'daemon composition root, pre-split, shrink-only; the +5 that crossed the cap is the hosted-session engine (one import and one call to composeHostedSessionsForFacade, plus the field on the returned runtime). It is composed HERE because the engine publishes its lifecycle channel through the control-plane gateway, and the gateway is built in this function, the same reason the approval broker\'s publisher and the companion chat manager are wired at this point. The composition detail lives in ./hosted-sessions-composition.ts, so nothing beyond the call site could be moved out of this file',
  },
  ...
  'packages/daemon-sdk/src/runtime-session-routes.ts': {
    ceiling: 898,
    justification: 'daemon runtime-session route surface, pre-split, shrink-only; +55 for the surface-collection wire (queued-for-surface response handling, sessions.inputs.deliver route + handler, sessions.inputs.list state/since cursor); +20 for the sessions.detach handler (surfaceId 400 guard + broker delegation); +5 for the task-retry ownership transfer (owner: spawnResult.id, so cancel-after-retry reaches the live agent) and the exhaustiveness assertion over the cancel|retry action union',
  },
  ...
  'packages/sdk/src/platform/control-plane/session-broker.ts': {
    ceiling: 888,
    justification: '+9 for hosted-turn liveness and the boot orphan sweep: the externalLivenessProbe field, its post-construction setter, the isExternallyLive line threaded into gcSweep, and a two-line applyBootOrphanSweep call at boot. The reaper used to judge liveness only by signals a HOSTED turn never emits, and closed a session \'idle-reaped\' at 21:45Z whose turn ran until 22:26Z; the probe is what lets it ask the engine instead of guessing. All of the sweep logic, the orphan predicate, the reason vocabulary and the disclosure live in session-broker-gc.ts, what is here is class state plus one call, which cannot move without inventing a second broker-shaped object to hold one nullable function. Original: control-plane session broker; handleIntent (the submit/steer/follow-up dispatch body) moved to the free function handleSharedSessionIntent in session-broker-intent.ts, following the deps-object convention of session-broker-gc.ts/session-broker-inputs.ts/session-broker-sessions.ts; the class keeps a thin delegating handleIntent wrapper; ceiling lowered to the post-split line count; +12 for the surface-notice port (field, setSurfaceNoticeSender, and the deps spread into handleIntent) that lets the broker say one line on a channel when it HEALS a route binding whose session was unusable. Without it the rollover is silent and reads as amnesia: the assistant that remembered the last two days answers the next message knowing nothing, and the owner cannot tell that from a fault. The healing itself, its policy and all its prose live in session-broker-intent.ts; what is here is three lines of plumbing and its doc, which cannot move without inventing a second broker-shaped object to hold one nullable function; +22 for failInput, the terminal counterpart to markInputDelivered(consumed: true). Without it a surface that collected an input and then failed to hand it to its loop had exactly one way to close the record, marking it completed, which writes down that the owner\'s message was answered when nothing received it. On a hosted session with nobody attached that was the only trace the message ever left. It sits beside cancelInput and uses the same updateSharedSessionInput/refresh/persist/publish sequence, so it cannot move out without taking that one with it',
  },
};

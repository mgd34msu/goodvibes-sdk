...
Fills the three control-plane gaps a surface hits the moment it stops sharing a
process — and a filesystem — with the daemon, plus the client's half of the
build-floor handshake. Every one of them is a thing the web UI could already do
over the wire and an in-process client could only do by reaching for its own
copy of a store the daemon actually owns.
